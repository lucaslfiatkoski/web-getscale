...
import { Power, Zap, Terminal, Lock, Key, Users, List, AlertTriangle, Menu, X, Phone, Mail, MapPin, Cpu, MessageCircle, Pin } from 'lucide-react';
...
// ======================================================
// NOVO COMPONENTE: CARD PARA TELA MOBILE (Melhora UX da Tabela)
// ======================================================
const WireColorCard = ({ row }) => (
    <div className="border border-gray-200 rounded-lg shadow-sm p-4 mb-3 bg-white">
        <div className="flex justify-between items-center mb-2 border-b pb-2">
            <span className="text-sm font-semibold text-gray-700 uppercase">Fio Acessório</span>
            <div className={`w-24 ${row.acessorioBg} ${row.acessorio === 'Branco' || row.acessorio === 'Amarelo' ? 'text-gray-900 border border-gray-300' : 'text-white'} px-2 py-1 rounded font-bold text-center text-sm`}>
                {row.acessorio}
            </div>
        </div>
        <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-semibold text-gray-700 uppercase">Fio Rastreador</span>
            <div className={`w-24 ${row.rastreadorBg} ${row.rastreador === 'Branco' || row.rastreador === 'NC' || row.rastreador === 'Amarelo' ? 'text-gray-900 border border-gray-300' : 'text-white'} px-2 py-1 rounded font-bold text-center text-sm`}>
                {row.rastreador}
            </div>
        </div>
        <div className="pt-2 border-t mt-2">
            <span className="text-sm font-semibold text-gray-700 uppercase block mb-1">Função</span>
            {/* Fonte reduzida para mobile: text-xs */}
            <p className="text-xs text-gray-600 leading-snug">{row.funcao}</p>
        </div>
    </div>
);
...
                  </h2>
                  
                  {/* VISUALIZAÇÃO DE CARD: Apenas para telas pequenas (mobile) */}
                  <div className="md:hidden">
                    {wireColors.map((row, index) => (
                        <WireColorCard key={index} row={row} />
                    ))}
                  </div>

                  {/* VISUALIZAÇÃO DE TABELA: Apenas para desktop (md e superior) */}
...
                          {/* Colunas com texto reduzido para otimização, como no código de exemplo */}
                          <th className="px-4 py-3 text-left font-semibold text-gray-700">Acessório</th>
                          <th className="px-4 py-3 text-left font-semibold text-gray-700">Função</th>
                          <th className="px-4 py-3 text-left font-semibold text-gray-700">Rastreador</th>
                        </tr>
                      </thead>
                      <tbody>
                        {wireColors.map((row, index) => (
                          <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                            {/* Cor Acessório: Aplicando padding responsivo para otimização de espaço */}
                            <td className="px-4 py-3">
                              <div className={`${row.acessorioBg} ${row.acessorio === 'Branco' || row.acessorio === 'Amarelo' ? 'text-gray-900' : 'text-white'} px-3 py-2 rounded font-medium text-center text-sm`}>
                                {row.acessorio}
                              </div>
                            </td>
                            {/* Função: Fonte reduzida para mobile (text-xs) no seu exemplo, mas mantemos text-base no desktop. */}
                            <td className="px-4 py-3 text-gray-700 text-base">{row.funcao}</td>
                            {/* Cor Rastreador */}
                            <td className="px-4 py-3">
                              <div className={`${row.rastreadorBg} ${row.rastreador === 'Branco' || row.rastreador === 'NC' || row.rastreador === 'Amarelo' ? 'text-gray-900' : 'text-white'} px-3 py-2 rounded font-medium text-center text-sm`}>
                                {row.rastreador}
                              </div>
                            </td>
                          </tr>
...
                        ⚙️ Passo 1 – Configuração no Software Multiportal
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
